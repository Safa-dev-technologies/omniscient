...
import { waitForJobCompletion, waitForSourceStatus, createMinimalPDF } from '../helpers.js';
...
describe('Data Integrity Integration', () => {
  ...
  it(
    'should maintain chunk count consistency',
    async () => {
      ...
      await waitForJobCompletion(
        embeddingQueue,
        result.sourceId,
        TEST_CONFIG.timeout.embedding * 2
      );
  ...
  it(
    'should assign vectorId to all chunks',
    async () => {
      const pdfBuffer = createMinimalPDF();
      const result = await knowledgeService.uploadDocument({
        tenantId: TEST_CONFIG.tenantId,
  ...
  it(
    'should delete all chunks and vectors when source is deleted',
    async () => {
      const pdfBuffer = createMinimalPDF();
      const result = await knowledgeService.uploadDocument({
        tenantId: TEST_CONFIG.tenantId,
  ...
  it(
    'should not create duplicate chunks on reindex',
    async () => {
      const pdfBuffer = createMinimalPDF();
      const result = await knowledgeService.uploadDocument({
        tenantId: TEST_CONFIG.tenantId,
  ...
  it(
    'should maintain referential integrity between chunks and sources',
    async () => {
      const pdfBuffer = createMinimalPDF();
      const result = await knowledgeService.uploadDocument({
        tenantId: TEST_CONFIG.tenantId,
  ...
  it(
    'should store correct metadata in chunks',
    async () => {
      const pdfBuffer = createMinimalPDF();
      const result = await knowledgeService.uploadDocument({
        tenantId: TEST_CONFIG.tenantId,
