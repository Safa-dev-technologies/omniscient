...
import { createMinimalPDF, waitForAllJobs, sleep, waitForSourceProcessing } from '../helpers.js';
...
describe('Rate Limiting Integration', () => {
  ...
  it('should delay jobs when concurrent limit is reached', async () => {
    ...
    const delayedForTenant = delayedJobs.filter(
      (j) => j.data.tenantId === TEST_CONFIG.tenantId
    );
    const activeForTenant = activeJobs.filter(
      (j) => j.data.tenantId === TEST_CONFIG.tenantId
    );
  ...
  it('should not starve other tenants', async () => {
    ...
    try {
      ...
      if (tenant2Source?.status) {
        expect(['EXTRACTING', 'CHUNKING', 'EMBEDDING', 'INDEXING', 'INDEXED', 'FAILED']).toContain(tenant2Source.status);
      }
    } finally {
      // Cleanup tenant 2
      try {
